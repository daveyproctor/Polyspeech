...
{
  ggplot(metaWTagsDF%>%filter(party %in% c("D","R")), aes(x=gender,y=get(emotion),fill=party)) + geom_boxplot() + theme(plot.title = element_text(hjust = 0.5)) + xlab("Gender") + ylab(emotion) + ggtitle(paste(emotion, "by party, gender"))
}
plotEmotion("anger")
plotEmotion("disgust")
plotEmotion("joy")
...
# Analysis
As one example, the effect of anger due to gender was not significant.
```{r, results='show'}
t.test(anger ~ gender, metaWTagsDF)
```
