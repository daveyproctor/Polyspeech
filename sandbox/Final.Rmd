...
metaDocsNearElectionsWPayoff %>% ggplot(aes(x=log(ret_all), y=vote_fraction, color=gender)) + geom_smooth() + geom_point()
```
...
metaDocsNearElections %>% inner_join(ibmDF) %>% 
  ...
  ggplot(aes(x=tone_name, y=log(ret_all), color=gender, fill=isHighScore)) + geom_boxplot() + ax
```
