...
metaDFWTweets %>%
  ...
  ggplot(aes(x=month, y=n, color=gender)) + geom_smooth() #+ geom_point() #+ geom_smooth() + 
  #ggtitle("Tweet Counts in Recent Years") + th

# Only run if needed; takes a while
...
```{r}
# Men attaching to this tweet technology faster than women
# Useless because diff num men and women
#summary(lm(n ~ day*gender, data=metaDFWTweets %>% group_by(gender, day=floor_date(created_at, "day")) %>%
#             filter(day>ymd("2012-1-1")) %>%
#             summarise(n=n())))

# Women actually availing themselves of Twitter faster than men
summary(lm(n ~ day*gender, data=metaDFWTweets %>% group_by(user_id, day=floor_date(created_at, "day")) %>%
  summarise(n=n(), gender=gender[[1]])))
```
...
ibmDF <- ibmDF %>% group_by(tone_name) %>%
...
```{r}
summary(aov(log(ret_all)~gender*isHighScore, data=metaDocsNearElections %>% inner_join(ibmDF) %>%
  filter(tone_name=="Openness")))
summary(aov(log(ret_all)~gender*isHighScore, data=metaDocsNearElections %>% inner_join(ibmDF) %>%
...
```{r}
rev(sort(table(hashtags_per_user$hashtag)))
```
...
```{r}
#tm::removeWords(metaDocsNearElections$doc, stopwords("en"))
#tweetsNearElections50
```
